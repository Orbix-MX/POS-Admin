 ...
 * Carga del catálogo y construcción de las tarjetas de producto.
 *
...
RG.catalogo = (function () {
  ...
  const RUTA_DATOS = 'data/plantas.json';
  ...
  /**
   * Obtiene el catálogo desde data/plantas.json.
   *
   * Bajo file:// el navegador bloquea fetch por CORS, así que se recurre al
   * respaldo `window.ROOTGARDEN_CATALOGO` que publica data/plantas.js. En un
   * servidor web siempre gana el JSON, que es la fuente de verdad.
   */
  const cargar = () => {
    if (promesaCarga) return promesaCarga;

    promesaCarga = fetch(RUTA_DATOS, { cache: 'no-cache' })
      .then((respuesta) => {
        if (!respuesta.ok) throw new Error(`HTTP ${respuesta.status}`);
        return respuesta.json();
      })
      .catch(() => {
        if (Array.isArray(window.ROOTGARDEN_CATALOGO)) return window.ROOTGARDEN_CATALOGO;
