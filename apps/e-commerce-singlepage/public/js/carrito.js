...
RG.carrito = (function () {
  ...
  const resolverProducto = (referencia) => {
    ...
    return (
      ...
      (RG.tienda && RG.tienda.productoPorId ? RG.tienda.productoPorId(id) : null)
    );
  };
  ...
  /** Descarta del pedido productos que ya no existen y refresca su precio. */
...
RG.carritoUI = (function () {
  ...
  const crearFila = (linea) => {
    ...
    return crear('li', { clase: 'pedido-item', 'data-linea': linea.id }, [
      crear('div', { clase: 'pedido-item__info' }, [
        crear('p', { clase: 'pedido-item__nombre', texto: linea.nombre }),
  ...
  const manejarEnvio = async () => {
    ...
    const numero = (window.RG_CONFIG && window.RG_CONFIG.whatsappNumber) || '';

    if (nodos.enviar) nodos.enviar.disabled = true;
    try {
      const folio = await RG.storeOrders.registrarPedido(telefono, lineas);
      RG.whatsapp.enviarPedido(numero, lineas, RG.carrito.totales(), folio);
    } finally {
