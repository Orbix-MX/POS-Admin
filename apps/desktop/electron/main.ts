...
function setupIpcHandlers() {
  ...
  ipcMain.handle('db:products:cache', (_e, products: unknown[]) => {
    cacheProducts(db, products as any[])
    return { ok: true }
  })
  ...
  ipcMain.handle('db:customers:cache', (_e, customers: unknown[]) => {
    cacheCustomers(db, customers as any[])
    return { ok: true }
  })

  // ── DB: Pending sales ──
  ipcMain.handle('db:sales:save-pending', (_e, sale: unknown) => {
  ...
  ipcMain.handle('db:cash:save-pending', (_e, movement: unknown) => {
  ...
  // ── Sync ──
  ipcMain.handle('sync:run', async (_e, opts: { apiBase: string; token: string }) => {
  ...
  // ── Print ──
  ipcMain.handle('print:ticket', async (_e, data: unknown) => {
    try {
      await printTicket(mainWindow!, data as any)
      return { ok: true }
    } catch (err) {
      return { ok: false, error: String(err) }
    }
  ...
  ipcMain.handle('print:pdf', async (_e, opts: { title: string; html: string }) => {
    try {
