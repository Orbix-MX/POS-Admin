...
/** `/(auth)/mfa-challenge` — the one screen reachable while `status === 'mfa-pending'`. */
...
export function RouteGuard({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    if (status === 'mfa-pending') {
      // Not just "any (auth) screen" — `sign-in` is also in that group, and
      // that's exactly the screen this has to navigate AWAY from.
      //
      // Cast: expo-router's typed routes are generated by the (broken-in-this-
      // sandbox) Expo CLI and haven't picked up this screen yet; the literal
      // path is correct and will typecheck cleanly once `expo start` runs.
      const onChallengeScreen = group === '(auth)' && (segments[1] as string) === 'mfa-challenge';
      if (!onChallengeScreen) router.replace(MFA_CHALLENGE_PATH as never);
      return;
    }
